 ...
 */
class BrickletOLED128x64 extends Device
{
    ...
    const DEVICE_IDENTIFIER = 263;

    const DEVICE_DISPLAY_NAME = 'OLED 128x64 Bricklet';
    ...
    /**
     * Appends 64 byte of data to the window as set by BrickletOLED128x64::newWindow().
     * 
     * Each row has a height of 8 pixels which corresponds to one byte of data.
     * 
     * Example: if you call BrickletOLED128x64::newWindow() with column from 0 to 127 and row
    ...
    /**
     * Clears the current content of the window as set by BrickletOLED128x64::newWindow().
     * 
     * 
     * @return void
     */
    ...
    /**
     * Returns the configuration as set by BrickletOLED128x64::setDisplayConfiguration().
     * 
     * 
     * @return array
     */
    ...
    public function writeLine($line, $position, $text)
    {
        ...
        for ($i = 0; $i < strlen($text) && $i < 26; $i++) {
            $payload .= pack('c', ord($text[$i]));
        }
        for ($i = strlen($text); $i < 26; $i++) {
            $payload .= pack('c', 0);
        }
